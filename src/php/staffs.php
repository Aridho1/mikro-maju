...
try {
    switch (M) {
        case 'login':
            ...
            setcookie(
                $cookieName,
                base64_encode(json_encode($_SESSION['auth'])),
                $cookieExpire,
                '/',
                '',
        ...
        case 'get-session':
            ...
            if (!empty($_SESSION['auth'])) {
                echo json_encode(['status' => true, 'auth' => $_SESSION['auth']]);
                die();
            }

            // jika session tidak ada, tapi cookie ada → buat session baru
            if (!empty($_COOKIE[$cookieName])) {
                $auth = json_decode(base64_decode($_COOKIE[$cookieName]), true);
