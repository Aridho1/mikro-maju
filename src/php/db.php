...
function uploadFile(string $name, string $tmpPath, ?string $filePrefixName = ''): array
{
    ...
    $newName = ($filePrefixName ?? '') . uniqid() . '.' . $exts;
...
function validateCheckboxInputByKeyName(string $key): int
{
    $GLOBALS[$key] = !!($GLOBALS[$key] ?? null) ? 1 : 0;
...
function putSSE(string $fileName, array $item)
{
    ...
    file_put_contents($eventFile, json_encode($eventObj, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES));
}
